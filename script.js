...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Crea il container per i messaggi se non esiste
  ...
  function validateForm(data) {
      ...
      if (!data.area_legale) {
          errors.push('Seleziona l\'area legale di tuo interesse');
      }
      
      if (!data.tipo_consulenza) {
          errors.push('Scegli il tipo di consulenza');
      }
      
      if (!data.descrizione || data.descrizione.length < 20) {
          errors.push('Descrivi la tua situazione (almeno 20 caratteri)');
      }
